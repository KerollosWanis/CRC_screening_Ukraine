    ...
    out <- out %>%
      mutate(
        next_possible_colonoscopy_age = case_when(
          ...
          negative_colonoscopy == 1 & hx_of_CRC == 0 ~ as.integer(next_possible_colonoscopy_age + 10),
          negative_colonoscopy == 1 & hx_of_CRC == 1 ~ as.integer(next_possible_colonoscopy_age + 5),
          ...
          (clinical_localized_CRC == 1 | clinical_regional_CRC == 1) &
            (years_since_dx == 1 | years_since_dx == 3) & dead == 0 ~ as.integer(age),
          (clinical_localized_CRC == 1 | clinical_regional_CRC == 1) &
            years_since_dx == 5 & dead == 0 ~ as.integer(age+1),
          clinical_localized_CRC_trt == 0 & clinical_regional_CRC_trt == 0 &
            clinical_localized_CRC == 0 & clinical_regional_CRC == 0 & 
...
apply_CRC_progression <- function(params, sim_data){
  ...
  with(params, {
    ...
    out <- out %>% mutate(
      ...
      #apply diagnostic colonoscopy for those who present with symptoms
      colonoscopy = case_when((clinical_distant_CRC == 1 & preclinical_distant_CRC == 1) |
                                (clinical_regional_CRC == 1 & preclinical_regional_CRC == 1) |
                                (clinical_localized_CRC == 1 & preclinical_early_CRC == 1) ~ as.integer(1),
                              TRUE ~ colonoscopy)
